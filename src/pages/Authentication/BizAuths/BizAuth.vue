...
function onCeoNameInput(e) {
  const filtered = e.target.value.replace(/[^ㄱ-ㅎ가-힣a-zA-Z\s]/g, '');
  e.target.value = filtered;
  ceoName.value = filtered;
}
...
.sub-content .sub-question img {
  width: 16px;
  height: 16px;
}
.sub-content .biz-input {
  width: 100%;
  height: 48px;
  margin-top: 8px;
  padding: 13px 12px;
...
.sub-content .biz-input::placeholder {
  color: #c2c2c2;
}
.sub-content:focus-within .biz-label {
  color: #2866ec;
}
.sub-content .biz-input:focus,
.sub-content:focus-within .biz-input {
  outline: none;
  border-color: #2866ec;
}
